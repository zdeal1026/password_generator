...
generateBtn.addEventListener("click", function () {
  pw = generatePassword();
  domcument.getElementById("password").placeholder = pw;
});
...
if (!confirmSmalls && !confirmBiggies && !confirmNumbers && !confirmSpecials) {
  select = alert("Please select something to build your password");
}
//user picks all options

else if (confirmSmalls && confirmBiggies && confirmNumbers && confirmSpecials) {
  select = character.concat(biggies, smalls, specials, numbers);
